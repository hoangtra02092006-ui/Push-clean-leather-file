...
| Dùng một phép đo pixel duy nhất cho "trắng là nền" | Nền trắng của ảnh JPG và **chữ trắng trong file vector** ở mức điểm ảnh giống hệt nhau: đều trắng, đều nằm cạnh chỗ trong suốt. Áp chung một phép loang thì chữ trắng bị bỏ theo — đo trên một thiết kế thật: dòng "BÒ TƯƠI" và dòng hotline ra file TIF **không có một điểm mực nào**, cả màu lẫn trắng. In lên áo tối là mất hai dòng chữ, mà mở trên nền trắng thì không ai thấy thiếu | Phân biệt bằng **có kênh trong suốt hay không**, không bằng điểm ảnh và cũng không bằng PDF/ảnh: file có kênh trong suốt (PDF, PNG đã tách nền) đã báo chỗ không in bằng chính độ trong suốt, nên trắng trong đó là nét vẽ. `PdfComposer.imageZones()` chỉ khoanh ảnh **dẹt** như JPG |
| Vẽ từng dải bằng `renderPageToGraphics` mà quên đặt **màu nền** | PDFBox xoá trang bằng màu nền của `Graphics2D`, mà mặc định của ảnh ARGB mới tạo là **đen đặc**. Cả tấm ra `ff000000` thay vì `00000000` — mất kênh alpha là mất luôn cách phân biệt "không có hình" với "hình màu đen", mà cả mặt nạ mực trắng lẫn lớp trong suốt đều dựa vào đó | `g.setBackground(new Color(0,0,0,0))` rồi `g.clearRect(...)` **trước** khi vẽ. `renderImageWithDPI` làm đúng việc này, nên nó trong suốt còn `renderPageToGraphics` thì không |
| Cắt dải bằng cách thu hẹp **CropBox** | Khung tính bằng point còn biên dải tính bằng điểm ảnh, nên mỗi dải lệch một phần điểm ảnh. Nét vẽ bị khử răng cưa khác đi: trên một tấm 6.732 × 9.673, kênh mực trắng lệch hẳn ở **206.114 điểm** (0,32%) — file vẫn mở bình thường, vẫn đủ năm kênh | Giữ nguyên CropBox, đẩy trang bằng `translate` **số nguyên** điểm ảnh. Hình vuông góc khi đó ghép lại đúng **từng byte**; chỉ nét cong còn lệch vài điểm ở rìa |
| Trả về `Raster` là **khung nhìn có offset** cho bộ ghi TIFF | Bộ ghi lấy thẳng `getDataBuffer().getData()` rồi đánh chỉ số **từ 0**, bỏ qua offset → mọi dải ghi ra nội dung của dải đầu. Đo trên ảnh 64 × 128 chia hai dải: số điểm lót trắng thành 8.192 thay vì 4.096, mà file vẫn mở bình thường | Chép phần cần ra **mảng riêng bắt đầu từ byte 0**. Test phải soi cả `getDataBuffer().getData().length`, không chỉ `getSample()` |
